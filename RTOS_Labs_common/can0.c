...
#define CAN_MAXFIFOSIZE 128      // maximum size of FIFO buffer
...
//
//*****************************************************************************
void CAN0_Handler(void){ uint8_t data[7];
    ...
    for(i = 0; i < 32; i++){    //test every bit of the mask
      if( (0x1 << i) & ulIDStatus){  // if active, get data
        ...
        if(xTempMsgObject.ulMsgID == RCV_ID){
					if(CAN_Fifo_Put(data[0]) == 0){
						CAN_DataLost++;
					} 
					if(CAN_Fifo_Put(data[1]) == 0){
						CAN_DataLost++;
					} 
					if(CAN_Fifo_Put(data[2]) == 0){
						CAN_DataLost++;
					} 
					if(CAN_Fifo_Put(data[3]) == 0){
						CAN_DataLost++;
					}
					if(CAN_Fifo_Put(data[4]) == 0){
						CAN_DataLost++;
					} 
					if(CAN_Fifo_Put(data[5]) == 0){
						CAN_DataLost++;
					} 
					if(CAN_Fifo_Put(data[6]) == 0){
						CAN_DataLost++;
					}
        }
      }
...
// in this case there is just one type, but you could accept multiple ID types
  CAN0_Setup_Message_Object(RCV_ID, MSG_OBJ_RX_INT_ENABLE, 7, NULL, RCV_ID, MSG_OBJ_TYPE_RX);
