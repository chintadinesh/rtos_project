...
uint32_t ST7735_DrawString(uint16_t x, uint16_t y, char *pt, int16_t textColor){
  ...
  if(y>15) return 0;
  while(*pt){
    OS_bWait(&LCDFree);       // aqcuire semaphore
    ST7735_DrawCharS(x*6, y*10, *pt, textColor, ST7735_BLACK, 1);
    OS_bSignal(&LCDFree);     // release semaphore
    pt++;
    x = x+1;
...
void ST7735_Message(uint32_t  device, uint32_t  line, char *string, int32_t value){
  uint32_t x = 0, y;
  char message[100];   // max characters per row is 21
  char padding[] = "                     ";   // padding clears previously printed characters

  if(line < 8 && device <= 1){    // only print for appropriate parameters
    ...
    // print message
	sprintf(message, "%.21s %d%.21s" , string, value, padding); // create message string
  ST7735_DrawString(x, y, message, ST7735_WHITE);
  }
}
