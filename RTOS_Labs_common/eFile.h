...
/**
 * \brief File Structure for Directory
 */   
struct file {
  char name[7];            // filename
  int16_t startBlock;      // starting block of file, -1 means unused
  uint16_t size;           // in bytes
};
typedef struct file fileType;

/**
 * \brief Block structure for File Allocation Table
 */  
struct block {
  int16_t nextBlock;       // next block of file, 0 means last block, -1 means unused
};
